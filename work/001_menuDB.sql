...
-- 1) 새로운 wanted계정 만들기
CREATE USER 'wanted'@'%' IDENTIFIED BY  'wanted'; -- 'localhost' 대신 '%'를 쓰면 외부 ip로 접속 가능하다.
...
GRANT ALL PRIVILEGES ON menudb.* TO 'wanted'@'%';	-- menu에 대한 모든 권한 부여

SHOW GRANTS FOR 'wanted'@'%';

-- 위까지 Root 계정에서 진행한다.
-- 아래는 새로운 wanted커넥션에서 진행

-- 3) 새로운 접속기 생성 후 접속하고 데이터베이스 활용하기
-- 좌측 상단의 home 버튼을 눌러 wanted계정 접속기를 만들어 접속하고 database(schema)를 사용한다.
-- 접속기의 Connection Name은 'wanted'로 지정
-- Parameters의 Username은 'wanted'로 지정(계정명)
